...
var path="http://localhost/suptrain_v2/php/"; 
$(document).ready(function(){
	...
	$("#maincontent").on('click','.oneSupervisor',function(){
		var supervisorID=$(this).attr('data-supervisorID');
		showSelectedSupervisor(supervisorID);
		$("#showInfo").modal();
	});
	$("#maincontent").on('click','.oneOrganization',function(){
		var organizationID=$(this).attr('data-organizationID');
		showSelectedOrganization(organizationID);
		$("#showInfo").modal();
	});
});
function showSelectedOrganization(organizationID){
	var target=path+'getOrganizations.php?id='+organizationID;
	txtData='';
	$.get(target,function(data){
	 if(data!='Fail'){
		var result=$.parseJSON(data);
		$.each(result.organizations,function(key,organization){
			txtData=txtData+'<p class="alert alert-info">';
			txtData=txtData+organization.name+', '+organization.address;
			txtData=txtData+'<br />'+organization.email+', '+organization.phone;
			txtData=txtData+'</p>';
		});

		$("#infobody").html(txtData);
	 }else $("#maincontent").html('<p class="alert alert-info">Login first!</p>');
	});

}
function showSelectedSupervisor(supervisorID){
	var target=path+'getSupervisors.php?id='+supervisorID;
	txtData='';
	$.get(target,function(data){
	 if(data!='Fail'){
		var result=$.parseJSON(data);
		$.each(result.supervisors,function(key,supervisor){
			txtData=txtData+'<p class="alert alert-info">';
			txtData=txtData+supervisor.firstname+' '+supervisor.lastname;
			txtData=txtData+'<br />'+supervisor.email+', '+supervisor.phone;
			txtData=txtData+'</p>';
		});
...
function showSupervisors(){
	...
	var txtData='<h5>Supervisors</h5><table class="table table-striped">';
	txtData=txtData+'<thead><tr><th>Name</th><th>Email</th><th>Phone</th></tr></thead>';
	txtData=txtData+'<tbody>';
	$.get(target,function(data){
	 if(data!='Fail'){
		var result=$.parseJSON(data);
		$.each(result.supervisors,function(key,supervisor){
			txtData=txtData+'<tr class="oneSupervisor" data-supervisorID='+supervisor.supervisorID+'>';
			txtData=txtData+'<td>'+supervisor.firstname+' ';
			txtData=txtData+supervisor.lastname+'</td>';
			txtData=txtData+'<td>'+supervisor.email+'</td>';
			txtData=txtData+'<td>'+supervisor.phone+'</td>';
			txtData=txtData+'</tr>';
		});
		txtData=txtData+'</tbody></table>';
...
function showOrganizations(){
	...
	var txtData='<h5>Training places</h5><table class="table table-striped">';
	txtData=txtData+'<thead><tr><th>Name</th><th>Address</th><th>Contactperson</th><th>Contact Info</th></tr></thead>';
	txtData=txtData+'<tbody>';
	$.get(target,function(data){
	 if(data!='Fail'){
		var result=$.parseJSON(data);
		$.each(result.organizations,function(key,organization){
			txtData=txtData+'<tr class="oneOrganization" data-organizationID="'+organization.organizationID+'">';
			txtData=txtData+'<td>'+organization.name+'</td>';
			txtData=txtData+'<td>'+organization.address+'</td>';
			txtData=txtData+'<td>'+organization.contactperson+'</td>';
			txtData=txtData+'<td>'+organization.email+', '+organization.phone+'</td>';
			txtData=txtData+'</tr>';
		});
		txtData=txtData+'</tbody></table>';
		$("#maincontent").html(txtData);
